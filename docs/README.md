...
## AN-DNA Audit Logging (Gate 2)

**Authoritative vs. UI Logs**
The AN-DNA Rust CLI produces two distinct output files during operation. 
* **`andna_audit.jsonl` (Authoritative):** This is the procurement-grade, tamper-evident Gate 2 artifact. It features a strictly monotonic sequence, a single numeric `audit_run_id` per session, and a `sha3-256` hash chain. **Evaluators and auditors must use this file for all integrity claims.**
* **`verification_log.json` (Non-Authoritative):** This is a legacy UI convenience file. It uses string-based run IDs and does not enforce cryptographic ordering. It should not be used for compliance validation.

**Sequence Convention**
The authoritative audit chain (`andna_audit.jsonl`) enforces a 0-based index. The genesis record of any session begins at `seq: 0` with a `prev_hash` of 64 zeros.
